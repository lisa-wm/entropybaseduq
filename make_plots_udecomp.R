...
make_point_cloud = function(dt, alphas = c(0.5, 1)) {
  ggplot(
    ...
    scale_color_manual(values = c("#414487FF", "#2A788EFF", "#7AD151FF")) +
...
# PLOT EMPIRICAL ENSEMBLE DISTRIBUTIONS ----------------------------------------

# result_dir = "/home/lisa-wm/Documents/1_work/1_research/repos/udecomp/results/"
# sample_sizes = c(1, 2, 5, 10, 50, 100)
# score_files_samples_mnist = lapply(
#   sample_sizes,
#   function(i) {
#     list_dt = lapply(
#       list.files(result_dir, sprintf("_%i_convnet_mnist(.?)*_scores", i)), 
#       function(j) {
#         dt = fread(paste0(result_dir, j), drop = 1, header = TRUE)
#         setnames(dt, sprintf("class_%i", 0:9))
#         dt[, obs := .I]
#       }
#     )
#     list_dt = lapply(
#       seq_along(list_dt), function(j) list_dt[[j]][, member := j]
#     )
#     do.call(rbind, list_dt) 
#   }
# )
# score_files_samples_cifar10 = lapply(
#   sample_sizes,
#   function(i) {
#     list_dt = lapply(
#       list.files(
#         result_dir, 
#         sprintf("_%i_efficientnetb7_cifar10_(.?)*_scores", i)
#       ), 
#       function(j) {
#         dt = fread(paste0(result_dir, j), drop = 1, header = TRUE)
#         setnames(dt, sprintf("class_%i", 0:9))
...
# )
# 
# plot_ensemble_distribution = function(dt, seed, n_obs = 3, title) {
#   set.seed(seed)
#   obs_plot = sample(dt$obs, n_obs)
#   dt_ens = dt[obs %in% obs_plot]
#   ens_pred = dt_ens[, lapply(.SD, mean), by = c("obs")][, member := -1]
#   ens_pred[
#     , entropy := -sum(.SD * log2(.SD)), 
#     .SDcols = sprintf("class_%i", 0:9),
#     by = obs
#   ][
#     , diversity := compute_diversity(.SD),
#     .SDcols = sprintf("class_%i", 0:9),
#     by = obs
#   ][
#     , rnk := frank(diversity, ties.method = "last")
#   ]
#   dt_plot = rbind(dt_ens, ens_pred[, -c("entropy", "diversity", "rnk")])
#   dt_plot[ , rnk := 0]
#   for (i in unique(dt_plot[, obs])) {
#     dt_plot[obs == i, "rnk"] = ens_pred[obs == i, "rnk"]
#   }
#   dt_plot = data.table::melt(
#     dt_plot, id.vars = c("obs", "member", "rnk")
#   )
#   ggplot2::ggplot(
#     dt_plot[, is_avg := as.factor(ifelse(member < 0, 1, 0))], 
#     aes(
#       x = variable, 
#       y = member, 
#       height = value, 
...
#     )
#   ) +
#     ggridges::geom_density_ridges(stat = "identity", alpha = 0.6) +
#     theme_bw() +
#     labs(x = "class", y = "ensemble member") +
#     scale_x_discrete(labels = 0:9) +
#     scale_y_discrete(breaks = c()) +
#     scale_fill_manual(
#       "mode",
#       values = c("darkgray", "blue"),
#       labels = c("individual", "ensemble")
#     ) +
#     facet_wrap(~rnk, ncol = 15) +
#     ggtitle(title)
# }
# 
# for (i in seq_along(sample_sizes)) {
#   p_1 = plot_ensemble_distribution(
#     dt = score_files_samples_mnist[[i]],
#     seed = i,
#     n_obs = 60,
#     title = sprintf("MNIST data, %i%% sample size", sample_sizes[i])
#   )
#   p_2 = plot_ensemble_distribution(
#     dt = score_files_samples_cifar10[[i]],
#     seed = i,
#     n_obs = 60,
#     title = sprintf("CIFAR10 data, %i%% sample size", sample_sizes[i])
#   )
#   ggsave(
#     paste0(
#       plot_dir,
#       sprintf("ens_distributions_mnist_%i.png", sample_sizes[i])
#     ),
#     p_1,
#     height = 10,
#     width = 15
...
#     paste0(
#       plot_dir,
#       sprintf("ens_distributions_cifar10_%i.png", sample_sizes[i])
#     ),
#     p_2,
#     height = 10,
#     width = 15
...
# dt_uvsood = lapply(
#     c("202302161852", "202302171034"),
#     function(i) {
#         dt = unique(
#             fread(
#                 paste(results_dir, "ood", i, "uvsood.csv", sep = "/"), 
#                 drop = 1
#             )
#         )
#         dt[, mean_value := mean(value), by = list(pl, bb, ds, case, metric)]
#         dt[, sd_value := sd(value), by = list(pl, bb, ds, case, metric)]
#         dt$ds[dt$ds == "mnist"] = "MNIST"
#         dt$pl[dt$pl == "de"] = "Ensemble"
#         dt$pl[dt$pl == "la"] = "Laplace"
#         dt
#     }
# )
# p = lapply(
...
#         make_theme(
#             ggplot(
#                 i[metric %in% c("tu", "au", "eu")], 
#                 aes(x = case, y = mean_value, fill = metric)
#             )
#         ) +
...
#                 values = c("cornflowerblue", "darkorange1", "black")
#             ) +
#             scale_x_discrete(labels = c("in-sample", "out-of-sample")) +
...
# 
# ggsave(
#     paste0(plot_dir, "uvsood_laplace.png"),
#     p[[1]],
#     height = 2,
#     width = 5
# )
...
# 
# 
# # PLOT UNCERTAINTY VS ENSEMBLE SIZE --------------------------------------------
# 
# dt_uvssize = unique(fread(paste0(plot_dir, "uvssize.csv"), drop = 1))
# dt_uvssize[, mean_value := mean(value), by = list(bb, ds, rs, size, metric)]
# dt_uvssize[, sd_value := sd(value), by = list(bb, ds, rs, size, metric)]
# dt_uvssize$ds[dt_uvssize$ds == "cifar10"] = "CIFAR10"
# dt_uvssize$ds[dt_uvssize$ds == "mnist"] = "MNIST"
# 
# p_size_mnist = make_theme(
#     ggplot(
#         dt_uvssize[ds == "MNIST" & bb == "efficientnetb3"],
#         aes(x = size, y = mean_value, col = metric)
#     )
# ) +
#     geom_point() +
#     geom_line() +
...
#         width = 0.5
#     ) +
#     labs(x = "ensemble size", y = "uncertainty") +
#     facet_wrap(~ ds, scales = "free_y", ncol = 2)
# p_size_mnist
# ggsave(
#   paste0(plot_dir, "uvssize_mnist.eps"), p_size_mnist, height = 2.5, width = 5
# )
# 
# p_size_cifar = make_theme(
#   ggplot(
#     dt_uvssize[ds == "CIFAR10"], aes(x = size, y = mean_value, col = metric)
#   )
# ) +
#   geom_point() +
#   geom_line() +
...
# p_size_cifar
# ggsave(
#   paste0(plot_dir, "uvssize_cifar.eps"), p_size_cifar, height = 2.5, width = 5
# )
# 
# p_size = make_theme(
#     ggplot(
#         dt_uvssize[bb != "resnet50"],
#         aes(x = size, y = mean_value, col = metric)
#     )
# ) +
#     geom_point() +
...
#     ) +
#     labs(x = "ensemble size", y = "uncertainty") +
#     facet_wrap(~ ds, scales = "free_y", ncol = 1)
# p_size
# ggsave(
#     paste0(plot_dir, "uvssize.eps"), p_size, height = 4, width = 5
# )
# 
# # PLOT SCORES ------------------------------------------------------------------
# 
...
#     function(i) {
#         list_dt = lapply(
#             list.files(result_dir, sprintf("_%i_efficient(.?)*_scores", i)), 
#             function(j) {
#                 dt = fread(paste0(result_dir, j), drop = 1, header = TRUE)
#                 setnames(dt, sprintf("class_%i", 0:9))
#                 dt[, obs := .I]
...
# )
# 
# plot_scores = function(dt, seed, n_obs = 3, title) {
#     set.seed(seed)
#     obs_plot = sample(dt$obs, n_obs)
#     dt_plot = dt[obs %in% obs_plot]
#     dt_plot = data.table::melt(dt_plot, id.vars = c("obs", "member"))
#     ggplot2::ggplot(
#         dt_plot,
#         aes(x = member, y = value, fill = as.factor(variable))) +
#         ggplot2::geom_col(position = "stack") +
#         facet_wrap(~obs, ncol = 10, strip.position = "left") +
#         scale_fill_brewer(palette = "Paired") +
#         theme(legend.position = "bottom") +
#         labs(x = "ensemble member", y = "uncertainty", title = title)
# }
# 
...
#         123, 
#         100, 
#         sprintf("%i%% of samples", sample_sizes[x])
#     )
#     ggsave(
#         paste0(plot_dir, sprintf("scores_%i.eps", x)), 
#         p,
#         height = 10,
#         width = 15
#     )
# }
# plot_scores(score_files[[1]], 123, 100, "5% of samples")
# plot_scores(score_files[[2]], 123, 100, "10% of samples")
# plot_scores(score_files[[3]], 123, 100, "25% of samples")
# plot_scores(score_files[[4]], 123, 100, "50% of samples")
# plot_scores(score_files[[5]], 123, 100, "100% of samples")
